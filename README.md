...
*   **🔀 本地 HLS 代理**：surrit 直链易 403，默认经 `curl_cffi` 本机代理转发给 N_m3u8DL-RE（16 线程），兼顾浏览器指纹与并发下载。
...
依赖含 **curl_cffi**（Chrome 指纹请求 surrit HLS）；未安装时会回退直连，可能遇到 403。详见 `missav-surrit-hls-403-local-proxy.md`。
...
- `app_ui.py`：侧栏导航 + 仪表盘首页（任务配置 / 优先级 / 终端 / 概览）及队列、历史、设置页
- `app_state.py`：任务统计、队列、历史持久化（`~/.missav_downloader/state.json`）
- `app_core.py`：爬取、下载与本地 HLS 代理逻辑
- `hls_local_proxy.py`：surrit 403 本机代理（curl_cffi 上游 + URL 改写）
- `虾片.py` / `虾片封装.py`：GUI 入口（开发 / 打包）
